...
if node -e 'const fs=require("fs");const base=Number(process.argv[1]);const tr=JSON.parse(fs.readFileSync(process.argv[2],"utf8"));const tone=JSON.parse(fs.readFileSync(process.argv[3],"utf8"));const st=JSON.parse(fs.readFileSync(process.argv[4],"utf8"));const trCoins=Number(tr.coinsLeft ?? tr.coinsRemaining ?? NaN);const toneCoins=Number(tone.coinsLeft ?? tone.coinsRemaining ?? NaN);const trPostId=String(tr.postId||"");const tonePostId=String(tone.postId||"");const stActive=String(st.activePostId||"");const trActive=String(tr.activePostId||"");const toneActive=String(tone.activePostId||"");const successPath=tr.ok===true && tone.ok===true && trPostId!=="" && tonePostId!=="" && trPostId!==tonePostId && trActive===trPostId && toneActive===tonePostId && stActive===tonePostId && Number.isFinite(base) && Number.isFinite(trCoins) && Number.isFinite(toneCoins) && trCoins===base-3 && toneCoins===base-5;const noCoinsPath=tr.ok===false && String(tr.error||"")==="Insufficient coins" && tone.ok===false && String(tone.error||"")==="Missing post" && stActive===trPostId && Number.isFinite(base) && base < 3;process.exit((successPath||noCoinsPath)?0:1)' "$COINS_BEFORE_OPTIONS" "$TMP/translate.json" "$TMP/tone.json" "$TMP/status-after-options.json"; then
  echo "PASS6"
else
  echo "FAIL6"
  echo "-- translate --"
